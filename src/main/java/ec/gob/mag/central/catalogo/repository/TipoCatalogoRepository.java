...
public interface TipoCatalogoRepository extends CrudRepository<TipoCatalogo, Long> {
	...
	@Query("SELECT t FROM TipoCatalogo t WHERE UPPER(t.tipocatNombre) NOT LIKE %?1% AND "
			+ "UPPER(t.tipocatDescripcion) NOT LIKE %?1% AND t.tipcatEliminado = false AND t.tipcatEstado=11")
